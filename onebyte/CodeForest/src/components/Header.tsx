...
export function Header() {
  ...
  useEffect(() => {
    fetchPublicCategoryTree()
      ...
      .catch((e) => console.error("헤더 카테고리 로딩 실패:", e));
  }, []);
              ...
              return (
                ...
                >
                  <button className="flex items-center gap-1 text-foreground hover:text-primary transition-colors py-2 px-3 rounded-lg hover:bg-secondary/50">
                    {category.groupName}
                    {hasSubs ? <ChevronDown className="w-4 h-4" /> : null}
                  </button>

                  {/* ✅ 0 렌더링 방지: length && 쓰지 말고 hasSubs 사용 */}
                  {hasSubs && hoveredCategory === category.groupId && (
        ...
        <div className="flex items-center gap-4">
          {!isLoggedIn ? (
            ...
            <>
              ...
              <Link to="/mypage" className="text-muted-foreground hover:text-foreground transition-colors">
                마이페이지
              </Link>
